...
% Αρχικοποίηση ευστοχιών για κάθε παίκτη στην πρώτη γενεά
accuracy = [repmat(p_1, 1, floor(N / 2)), repmat(p_2, 1, ceil(N / 2))];
...
for experiment = 1:num_experiments
    ...
    for generation = 1:J
        ...
        %for player = 1:N
        %   fprintf('  Παίκτης %d: Πιθανότητα ευστοχίας = %.2f, Στρατηγική = %s\n', player, accuracy(player), str_vector{player});
        %end
        ...
        for i = 1:N-1
            for j = i+1:N
                % Καθορισμός της πιθανότητας ευστοχίας για κάθε παίκτη από το αρχικό διανύσμα accuracy
                p_i = accuracy(i);
                p_j = accuracy(j);

                % Διεξαγωγή του ματς μεταξύ του παίκτη i και του παίκτη j
                [payoff_i, payoff_j, ~, ~] = Duel(i, j, T, @SelectStrategy, @Strat, str_vector, a, b, p_i, p_j);
...
% Συνάρτηση Duel
function [payoff_p1, payoff_p2, state_p1, state_p2] = Duel(p1, p2, T, SelectStrategy, Strat, str_vector, a, b, prob1, prob2)
...
for t = 1:T
    action_i = Strat(memory_i, str1, prob1); % Δράση για τον παίκτη 1
    action_j = Strat(memory_j, str2, prob2); % Δράση για τον παίκτη 2

    % Αν ο παίκτης i πυροβολεί τον j
    if action_i && rand < prob1
        state_p2 = 0; % Ο παίκτης j σκοτώνεται
        killFlag(1) = 1;
    end

    % Αν ο παίκτης j πυροβολεί τον i
    if action_j && rand < prob2
